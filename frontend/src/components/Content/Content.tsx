...
function Content() {
  ...
  const [deleteContentId, setDeleteContentId] = useState<ObjectId | undefined>(undefined);
...
const handleEditClick = (contentId?: ObjectId) => {
  ...
  const contentToEdit = contentList.find((content) => content._id === contentId);
  if (contentToEdit) {
...
const handleEditSubmit = async () => {
  if (editedContent._id) {
    ...
    try {
      const response = await fetch(`${backendURL}/update-content/${editedContent._id}`, {
        method: "PUT",
        headers: {
...
//fetch content
  ...
  const handleDeleteConfirm = async () => {
    ...
    if (deleteContentId) {
      const backendURL = envVariables.backendURL;
      try {
        const response = await fetch(`${backendURL}/delete-content/${deleteContentId}`, {
          method: "DELETE",
        });
